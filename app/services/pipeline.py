...
def generate_memorial_glp_v2(
    raw_payload: dict[str, Any],
    output_path: Path,
) -> PipelineResult:
    context = build_memorial_glp_v2_context(raw_payload)
    validate_memorial_glp_v2_context(context)
    rendered_output_path = render_memorial_glp_v2(context, output_path)
    return PipelineResult(context=context, output_path=rendered_output_path)
